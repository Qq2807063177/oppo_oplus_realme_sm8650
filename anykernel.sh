...
if [ -f "$AKHOME/zram.zip" ]; then
    ZRAM_PATH="$AKHOME/zram.zip"
    ui_print "  -> 找到 ZRAM 模块: zram.zip"
else
    ui_print "  -> 未找到 ZRAM 模块，跳过安装"
    ZRAM_PATH=""
fi

if [ -n "$ZRAM_PATH" ]; then
    ui_print "是否安装 ZRAM 模块来添加更多内核支持的算法(默认zstd单8G)?"
    ui_print "音量上键：跳过安装"
    ui_print "音量下键：安装模块"
    ...
    zram_key=""
    while [ "$zram_key" = "" ]; do
        zram_key=$(getevent -qlc 1 | awk '{ print $3 }' | grep 'KEY_VOLUME')
        sleep 0.2
    done
    case "$zram_key" in
        "KEY_VOLUMEDOWN")
            if [ -f "/data/adb/ksud" ]; then
                ...
                /data/adb/ksud module install "$ZRAM_PATH"
                ui_print "ZRAM 模块安装完成"
            else
                ui_print "未找到 KSUD，跳过 ZRAM 安装"
            fi
            ;;
        "KEY_VOLUMEUP")
